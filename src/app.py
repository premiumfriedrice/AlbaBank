...
class MainMenu(ctk.CTkTabview):
    def __init__(self, master, **kwargs):
        ...
        self.depositLabel = ctk.CTkLabel(self.checkingBalanceFrame, text="Deposit", font=("IBMPlexSerif", 15), justify=ctk.CENTER)
        self.depositLabel.pack(padx=30, pady=30)
        ...
        self.TransactionsSavingsFrame = ctk.CTkScrollableFrame(self.savingsBalanceFrame, label_text="Transactions")
        self.TransactionsSavingsFrame.pack(side=ctk.BOTTOM, padx=20, pady=20, ipadx=20, ipady=20, fill="both", expand=True)
        ...
        self.AccountChoice = ctk.CTkOptionMenu(self.depositFrame, values=["Checking", "Savings"], font=("IBMPlexSerif", 20), width=200, height=30)
        self.AccountChoice.pack(padx=60, pady=20)
        self.amountEntry = ctk.CTkEntry(self.depositFrame, placeholder_text="0.00", width=200, height=40)
        self.amountEntry.pack(padx=60, pady=20)
        self.depositButton = ctk.CTkButton(self.depositFrame, text="Deposit", width=80, height=40)
        self.depositButton.pack(padx=60, pady=20)
...
class App(ctk.CTk):
    def __init__(self, *args, **kwargs):
        ...
        self.username = "Bamaboy21"
        self.password = "123password"
    ...
    def login(self):
        if self.usernameEntry.get() == '' or self.passwordEntry.get() == '':
            messagebox.showwarning(title="Error", message="Please enter your username and password.")
        elif self.username == self.usernameEntry.get() and self.password == self.passwordEntry.get():
            self.openMainMenu()
